...
from NAJMI_ADMIN import DEcwHisPErMsG_pb2 , MajoRLoGinrEs_pb2 , PorTs_pb2 , MajoRLoGinrEq_pb2 , sQ_pb2 , Team_msg_pb2, RemoveFriend_Req_pb2, GetFriend_Res_pb2, spam_request_pb2, devxt_count_pb2, dev_generator_pb2, kyro_title_pb2, room_join_pb2
...
region = 'IN'
WHITELISTED_UIDS = {
    "Najme_Alam❤️", # don't change this text
    "5513136279"
}
...
def load_credentials_from_file(filename="NAJMI_ADMIN.txt"):
    """
    Load UID and password from NAJMI_ADMIN.txt file
    """
    try:
        if not os.path.exists(filename):
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]NAJMI_FF_EXPERIMENT [00FF00]___ NAJMI_FF_EXPERIMENT____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : NAJMI_FF_EXPERIMENT\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
             f"https://like-api-by-najmi-admin.vercel.app/like?uid={uid}&server_name=ind",
             timeout=15
             )
...
def fake_likes(uid):
    try:
        likes_api_response = requests.get(
             f"https://your_like_domins.vercel.app/like?uid={uid}&server_name=ind",
             timeout=15
             )
...
def send_visits(player_id):
    ...
    url = f"https://visit-api.vercel.app/visit?uid={player_id}&region=bd"
    try:
...
def talk_with_ai(question):
    url = f"https://your-api.vercel.app/ai/ask?key=MG24_GAMER&question={question}"
    res = requests.get(url)
    if res.status_code == 200:
        data = res.json()
...
def spam_requests(player_id):
    url = f"https://spam-api.vercel.app/spam?uid={player_id}&region=bd"
...
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000]NAJMI_FF_EXPERIMENT
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
    fields = {
    1: 5,
    2: {
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde NAJMI_FF_EXPERIMENT ya meku agar tu noob bolra toh tu g a y hai
...
"""
                                ...
                                credentials = load_credentials_from_file("NAJMI_ADMIN.txt")
                                if not credentials:
...
[b][i][FFC0CB]youtube: NAJMI_FF_EXPERIMENT 99[/b]
...
#==================≈===========  /HELP MENU COMMANDS ========================================
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "hello", "hi"):
                            ...
                            header = f"[C][B][FF1493]Hey User Welcome To [00FFAA]NAJMI_FF_EXPERIMENT's BOT"
                            await safe_send_message(response.Data.chat_type, header, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu1 = """[C][B][FFD700]═══⚡COMMAND MENU 1-8⚡═══[00AAFF][B]
├─ [00AAFF]⚡Help Menu 
│  └─ [FF0000]/help
...
[00AAFF]━━━━━━━━━━━━[FF0000]"""
                            ...
                            menu2 = """[C][B][FFD700]═══⚡COMMAND MENU 9-16⚡═══[00AAFF][B]
├─ [00AAFF]Send Likes To Account⚡
│  └─ [FF0000]/like [uid]
├─ [00AAFF]Player Information 
│  └─ [FF0000]/info [uid]
├─ [00AAFF]Get Player Bio
│  └─ [FF0000]/bio [uid]
├─ [00AAFF]Account Level Info
│  └─ [FF0000]/level [uid]
├─ [00AAFF]TikTok Id Info
│  └─ [FF0000]/tt [username]
├─ [00AAFF]YouTube Channel Info
│  └─ [FF0000]/yt [name]
├─ [00AAFF]Guild Information 
│  └─ [FF0000]/guild [guild_id]
└─ [00AAFF]Get Player Status 
   └─ [FF0000]/status [uid]
[00AAFF]━━━━━━━━━━━━[FF0000]"""
                            await safe_send_message(response.Data.chat_type, menu2, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu3 = """[C][B][FFD700]═══⚡COMMAND MENU 17-24⚡═══[00AAFF][B]
├─ [00AAFF]Chat With Ai
│  └─ [FF0000]/ai [question]
├─ [00AAFF]Account Ban Check 
│  └─ [FF0000]/check [uid]
...
├─ [00AAFF]Gali Friend 
│  └─ [FF0000]/gali [name]
├─ [00AAFF]Direct Message 
│  └─ [FF0000]/dm [uid] [msg]
├─ [00AAFF]Message Spam 
│  └─ [FF0000]/msg [msg] [repeat]
└─ [00AAFF]Wave Messages 
   └─ [FF0000]/mg [msg] [repeat]
[00AAFF]━━━━━━━━━━━━[FF0000]"""
                            await safe_send_message(response.Data.chat_type, menu3, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu4 = """[C][B][FFD700]═══⚡COMMAND MENU 25-32⚡═══[00AAFF][B]
├─ [00AAFF]Bundle List
│  └─ [FF0000]/bundle 
├─ [00AAFF]Set Bundle 
│  └─ [FF0000]/bundle [name]
├─ [00AAFF]Lag Team 
│  └─ [FF0000]/lag [Team Code]
├─ [00AAFF]Attack Team 
│  └─ [FF0000]/attack [Team Code]
├─ [00AAFF]Level Up Bot
│  └─ [FF0000]/lw [Team Code]
├─ [00AAFF]Stop Level Up Bot 
│  └─ [FF0000]/stop
├─ [00AAFF]Developer Info
│  └─ [FF0000]/dev
└─ [00AAFF]Admin Info
   └─ [FF0000]/admin
[00AAFF]━━━━━━━━━━━━[FF0000]"""
                            ...
                            menu5 = """[C][B][FFD700]═══⚡COMMAND MENU 33-38⚡═══[00AAFF][B]
├─ [00AAFF]Bundle Spam 
│  └─ [FF0000]/bb_lag [Team Code]
├─ [00AAFF]Tcp Store
│  └─ [FF0000]/store
├─ [00AAFF]Spam Invite 
│  └─ [FF0000]/spm_inv [uid]
...
│  └─ [FF0000]/spam_req [uid]
├─ [00AAFF]Spam Join Requests 
│  └─ [FF0000]/spam_join [uid]
├─ [00AAFF]Join Req With Badge 'C'
│  └─ [FF0000]/s1 [uid]
├─ [00AAFF]Join Req With Badge 'V'
│  └─ [FF0000]/s2 [uid]
└─ [00AAFF]Join Req With Badge 'FF'
   └─ [FF0000]/s3 [uid]
[00AAFF]━━━━━━━━━━━━[FF0000]"""
                            await safe_send_message(response.Data.chat_type, menu5, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu6 = """[C][B][FFD700]═══⚡COMMAND MENU 39-46⚡═══[00AAFF][B]
├─ [00AAFF]Join Req With Badge 'Old V'
│  └─ [FF0000]/s4 [uid]
├─ [00AAFF]Join Req With Badge 'Pro'
│  └─ [FF0000]/s5 [uid]
├─ [00AAFF]Bot Speed ⚡⚡⚡
│  └─ [FF0000]/speed
├─ [00AAFF]Evo Cycle Self 
│  └─ [FF0000]@evos
├─ [00AAFF]Evo Cycle Player 
│  └─ [FF0000]@evos [uid]
├─ [00AAFF]Stop Evo Cycle 
│  └─ [FF0000]@sevos
├─ [00AAFF]Play Emote Self
│  └─ [FF0000]/e [emote]
└─ [00AAFF]Play Emote Player 
   └─ [FF0000]/e [uid] [emote]
[00AAFF]━━━━━━━━━━━━[FF0000]"""
                            await safe_send_message(response.Data.chat_type, menu6, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu7 = """[C][B][FFD700]═══⚡COMMAND MENU 47-54⚡═══[00AAFF][B]
├─ [00AAFF]Play Emote Without Bot
│  └─ [FF0000]/me [Team Code] [emote]
├─ [00AAFF]Player Emote Without Bot
│  └─ [FF0000]/me [tc] [uid] [emote]
├─ [00AAFF]Play Evo Emote 
│  └─ [FF0000]/evo [uid] [1-21]
├─ [00AAFF]Play Fast Emote 25x
│  └─ [FF0000]/fast [uid] [emote id]
├─ [00AAFF]Off/Mute Bot (admin)
│  └─ [FF0000]/off
├─ [00AAFF]On/Unmute Bot (admin)
│  └─ [FF0000]/unmute 
├─ [00AAFF]Block User (admin)
│  └─ [FF0000]/block [uid]
└─ [00AAFF]Unblock User (admin)
   └─ [FF0000]/unblock [uid]
[00AAFF]━━━━━━━━━━━━[FF0000]"""
                            await safe_send_message(response.Data.chat_type, menu7, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu8 = """[C][B][FFD700]═══⚡COMMAND MENU 55-62⚡═══[00AAFF][B]
├─ [00AAFF]Good Message To Player 
│  └─ [FF0000]/praisa [name]
├─ [00AAFF]Send Fake Likes⚡
│  └─ [FF0000]/like [uid]
├─ [00AAFF]Item Id To Info
│  └─ [FF0000]/item [item_id]
├─ [00AAFF]Super Calculator 
│  └─ [FF0000]/math [expression / 2+3]
├─ [00AAFF]EVENT DETAILS 
...
[00AAFF]━━━━━━━━━━━━[FF0000]"""
                            ...
                            menu9 = """[32CD32][B]═══⚡ AUTOMATIC & TIPS ⚡═══
[00AAFF]Auto Accept Team Invite
[FF0000]Auto Reply 'noob'
[00AAFF]Auto Play Ring After Join
[FF0000]Auto Ring For Inviter
[00AAFF]/sticker & /title Work In Only Team Chat
[FF0000]If Any Problems, 
[00AAFF]Dm Owner In Telegram NAJMI_FF_EXPERIMENT 
[00AAFF]━━━━━━━━━━━━"""
                            await safe_send_message(response.Data.chat_type, menu9, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)
                        response = None
...
async def MaiiiinE():
    ...
    print("📁 Loading credentials from NAJMI_ADMIN.txt...")
    credentials = load_credentials_from_file("NAJMI_ADMIN.txt")
    
    if not credentials:
        print("❌ Failed to load credentials!")
        print("💡 Please create NAJMI_ADMIN.txt with your UID and password")
        print("📝 Format: uid=4294595780,password=0710DCBA4500D479C677C072B7E36D8E974888CE1C3D4BB8674D814CF895C6AE")
        return None
